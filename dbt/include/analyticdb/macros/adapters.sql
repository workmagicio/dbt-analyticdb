...
{% macro analyticdb__list_schemas(database) %}
...
{% macro analyticdb__create_schema(relation) -%}
  {%- call statement('create_schema') -%}
    create schema if not exists {{ relation.without_identifier() }}
  {%- endcall -%}
{% endmacro %}

{% macro analyticdb__drop_schema(relation) -%}
  {%- call statement('drop_schema') -%}
    drop schema if exists {{ relation.without_identifier() }}
  {% endcall %}
{% endmacro %}

{% macro analyticdb__drop_relation(relation) -%}
    {% call statement('drop_relation', auto_begin=False) -%}
        drop {{ relation.type }} if exists {{ relation }}
    {%- endcall %}
{% endmacro %}

{% macro analyticdb__truncate_relation(relation) -%}
    {% call statement('truncate_relation') -%}
      truncate table {{ relation }}
    {%- endcall %}
{% endmacro %}

{% macro analyticdb__create_table_as(temporary, relation, sql) -%}
...
{% macro analyticdb__current_timestamp() -%}
  current_timestamp()
{%- endmacro %}

{% macro analyticdb__rename_relation(from_relation, to_relation) -%}
  {#
...
{% macro analyticdb__list_relations_without_caching(schema_relation) %}
...
{% macro analyticdb__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro analyticdb__get_phony_data_for_type(data_type) %}
  {#
...
{% macro analyticdb__get_empty_schema_sql(columns) %}
    {%- set col_err = [] -%}
    select
    {% for i in columns %}
      {%- set col = columns[i] -%}
      ...
      {{ analyticdb__get_phony_data_for_type(col['data_type']) }} as {{ col_name }}{{ ", " if not loop.last }}
    {%- endfor -%}
    {%- if (col_err | length) > 0 -%}
